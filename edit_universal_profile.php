...
$user_id = isset($_GET['id']) ? intval($_GET['id']) : $_SESSION['admin_id'];
...
<body>
                ...
                <div class="dashboard_content_main">
                    <div class="container mt-4">
                        ...
                        <div class="card shadow-sm">
                            ...
                            <div class="card-body">
                                <form action="update_universal_profile.php" method="POST" enctype="multipart/form-data" id="profileForm">
                                    ...
                                    <!-- Include a hidden field for manual outlet value -->
                                    <input type="hidden" id="manual_outlet_value" name="manual_outlet_value" value="<?= htmlspecialchars($user_data['distrib_outlet'] ?? '') ?>">
    ...
    fetch('philippine_places_api.php')
        ...
        .then(data => {
            ...
            var clearSearch = document.getElementById('clear_outlet_search');
            var outletDebug = document.getElementById('outletDebug');
            var allOptions = []; // Will store all options for search filtering
            ...
            // Helper function to validate if a string is a valid Philippine place
            function isValidPlace(place) {
                if (!place) return false;
            ...
            if (<?= $user_type === 'distributor' ? 'true' : 'false' ?>) {
                document.querySelector('form').addEventListener('submit', function(e) {
                    ...
                    if (!outletValue || outletValue.trim() === '') {
                        ...
                        select.focus();
                    } else if (!isValidPlace(outletValue)) {
                        e.preventDefault();
                        alert('Please select a valid Philippine location for outlet.');
                        select.focus();
                    } else {
                        // Create a hidden field to store the manual outlet value for better form handling
                        var manualOutletField = document.createElement('input');
                        manualOutletField.type = 'hidden';
                        manualOutletField.name = 'manual_outlet_value';
                        manualOutletField.value = outletValue;
                        this.appendChild(manualOutletField);
                ...
                data.forEach(function(region) {
                    ...
                    region.provinces.forEach(function(province) {
                        ...
                        province.cities.forEach(function(city) {
                            ...
                            var optText = `${regionName} - ${provinceName} - ${city}`;
                            allOptions.push({ value: optText, label: optText });
                        });
                    });
                });
                
                console.log(`Generated ${allOptions.length} place options from Philippine Places API`);
            } else {
            ...
            if (select) {
                ...
                // Search functionality for outlet dropdown
                if (outletSearch) {
                    var searchTimeout;
                    ...
                    outletSearch.addEventListener('input', function() {
                        ...
                        // Debounce the search
                        searchTimeout = setTimeout(function() {
                            // For empty search, restore all options
                            if (!searchTerm) {
                                ...
                                // Add selected value if exists
                                if (selectedValue && isValidPlace(selectedValue)) {
                                    select.innerHTML += `<option value="${selectedValue}" selected>${selectedValue}</option>`;
                                }
                                
                                // Repopulate all options by region
                                sortedRegions.forEach(function(region) {
                                    regionGroups[region].forEach(function(opt) {
                                        // Don't duplicate selected value
                            ...
                            // First try direct API search for more accurate results
                            fetch(`philippine_places_api.php?search=${encodeURIComponent(searchTerm)}`)
                                ...
                                .then(searchData => {
                                    ...
                                    if (Array.isArray(searchData)) {
                                        searchData.forEach(function(region) {
                                            var regionName = region.region;
                                            ...
                                            region.provinces.forEach(function(province) {
                                                ...
                                                province.cities.forEach(function(city) {
                                                    // Format as "Region - Province - City"
                                                    var optText = `${regionName} - ${provinceName} - ${city}`;
                                                    searchResults.push({ value: optText, label: optText });
                                                });
                                            });
                                        });
                                    }
                                    
                                    // Reset select with search results
                                    select.innerHTML = '<option value="">Select a place in the Philippines</option>';
                                    ...
                                    if (searchResults.length > 0) {
                                        ...
                                        searchResults.forEach(function(opt) {
                                            // Don't duplicate selected value
                                            if (opt.value !== selectedValue) {
                                                select.innerHTML += `<option value="${opt.value}">${opt.label}</option>`;
                                            }
                                        ...
                                        if (outletDebug) {
                                            outletDebug.innerHTML = `<small class="text-success">Found ${searchResults.length} matches for "${searchTerm}"</small>`;
                                            // Hide after 3 seconds
                                            setTimeout(function() {
                                                outletDebug.style.display = 'none';
                                            }, 3000);
                                ...
                                .catch(error => {
                                    ...
                                    var selectedValue = select.value;
                                    var filteredOptions = allOptions.filter(opt => 
                                        opt.label.toLowerCase().includes(searchTerm.toLowerCase())
                                    );
                                    
                                    // Reset select with filtered results
                                    select.innerHTML = '<option value="">Select a place in the Philippines</option>';
                                    ...
                                    if (outletDebug) {
                                        outletDebug.innerHTML = `<small class="text-warning">Search API failed. Found ${filteredOptions.length} local matches for "${searchTerm}"</small>`;
                                        // Hide after 3 seconds
                                        setTimeout(function() {
                                            outletDebug.style.display = 'none';
                                        }, 3000);
                ...
                if (clearSearch) {
                    clearSearch.addEventListener('click', function() {
                        if (outletSearch) {
                            outletSearch.value = '';
                            // Trigger input event to reset the dropdown
                            var event = new Event('input', {
                                'bubbles': true,
                                'cancelable': true
                            });
                            outletSearch.dispatchEvent(event);
                        }
                    });
                }
        ...
        .catch(error => {
            ...
            if (select) {
                ...
                const commonPlaces = [
                    ...
                    "Bangsamoro Autonomous Region - Tawi-Tawi - Bongao" 
                ];
                ...
                // Disable search functionality since API failed
                if (outletSearch) {
                    outletSearch.disabled = true;
                    outletSearch.placeholder = "Search unavailable (API error)";
                }
