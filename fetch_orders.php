...
try {
    ...
    $sql = "SELECT o.order_id, o.customer_name, o.customer_contact, o.customer_address, o.total_amount, o.status, o.created_at, o.updated_at, o.points, o.user_type, 
            CONCAT(a.admin_fname, ' ', IFNULL(a.admin_mname, ''), ' ', a.admin_lname) as created_by_name,
            COALESCE(oi.item_count, 0) as item_count,
            COALESCE(oi.total_quantity, 0) as total_quantity,
            COALESCE(oi.avg_unit_price, 0) as unit_price
            FROM orders o 
            LEFT JOIN admin_signup a ON o.created_by = a.admin_id 
            LEFT JOIN (
    ...
    // If staff is logged in, filter orders if needed
    if ($staff_id > 0 && isset($_SESSION['staff_id'])) {
        // If orders table has a created_by or staff_id column, filter by it
    ...
    if (!empty($search)) {
        $conditions[] = "(o.order_id LIKE ? OR o.customer_name LIKE ? OR o.customer_contact LIKE ?)";
        $searchParam = '%' . $search . '%';
        $params[] = $searchParam;
        $params[] = $searchParam;
        $params[] = $searchParam;
...
// Fetch admin name by admin_id (example usage)
function getAdminName($conn, $admin_id) {
    ...
    $stmt->close();
    if ($admin) {
        return $admin['admin_fname'] . ' ' . $admin['admin_mname'] . ' ' . $admin['admin_lname'];
    } else {
        return "Admin";
    }
}
