<?php
// Start session to ensure $_SESSION['staff_id'] is available
          ...
          <table>
            ...
            <tbody>
              ...
              if ($staffResult && $staffResult->num_rows > 0) {
                  $count = 1;
                  $currentStaffId = isset($_SESSION['staff_id']) ? $_SESSION['staff_id'] : null;
                  while ($row = $staffResult->fetch_assoc()) {
                      $isCurrent = ($currentStaffId && $row['staff_id'] == $currentStaffId);
                      echo "<tr>";
                      echo "<td>" . $count++ . "</td>";
                      ...
                      echo "<td>";
                      echo "<button class='btn btn-sm btn-info' onclick='viewStaff(" . $row['staff_id'] . ")' data-bs-toggle='tooltip' title='View Staff Details'>";
                      echo "<i class='fa fa-eye text-white'></i>";
                      echo "<button class='btn btn-sm btn-primary' onclick='editStaff(" . $row['staff_id'] . ")' data-bs-toggle='tooltip' title='Edit Staff' style='margin-left:5px;'>";
                      echo "<i class='fa fa-edit'></i> ";
                      echo "</button>";
                      if ($isCurrent) {
                          echo " <span class='badge bg-success' style='margin-left:5px;'>Current User</span>";
                      }
                      echo "</button>";
                      echo "</td>";
                      echo "</tr>";
...
<?php if (isset($_GET['new_staff_id'])): ?>
<script>
document.addEventListener('DOMContentLoaded', function() {
  var row = document.getElementById('newStaffRow');
  if (row) {
    row.scrollIntoView({behavior: 'smooth', block: 'center'});
    Swal.fire({
      icon: 'success',
      title: 'New Staff Added',
      text: 'The new staff member has been added and highlighted.',
...
function viewStaff(staffId) {
  ...
  fetch(`s_get_staff_details.php?staff_id=${staffId}`)
...
function editStaff(staffId) {
  window.location.href = 's_edit_universal_profile.php?type=staff&id=' + staffId;
}
