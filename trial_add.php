...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$category) $errors[] = "Category selection is required.";
    ...
    if (!$errors) {
        ...
        if (move_uploaded_file($image['tmp_name'], $destination)) {
            ...
            // Use consistent column names (quantity instead of stock, product_image instead of image)
            $stmt = $mysqli->prepare("INSERT INTO products (product_name, category, description, price, quantity, product_image) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->bind_param('sssdis', $product_name, $category, $description, $price, $stock, $destination);
            if ($stmt->execute()) {
                $stmt->close();
                
                echo "<script>
                    alert('Product added successfully!');
                    window.location.href = 'trial_view.php';
                </script>";
                exit;
            } else {
